"""
Reused from https://github.com/aturanjanin/pvclust
"""
...
from scipy.cluster.hierarchy import (dendrogram, set_link_color_palette, leaves_list, to_tree, linkage)
...
class PvClust:
    ...
    def plot(self, labels=None, output_directory=None, file_name='PV_clust'):
        """Plot dendrogram with AU BP values for each node"""
        plot_dendrogram(self.linkage_matrix,
                        np.array(self.result[['AU', 'BP']]), labels, output_directory, file_name)
...
def plot_dendrogram(linkage_matrix, pvalues, labels=None, output_directory=None,
                    file_name='PV_clust'):
    """ Plot dendrogram with AU BP values for each node"""
    ...
    plt.xticks(rotation=90, ha='right', fontsize=8)  # Rotate labels
    plt.subplots_adjust(bottom=0.3)  # Add space at the bottom for long labels
